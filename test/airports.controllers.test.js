...
describe('Airports Controller Database Mocking Unit Tests', () => {
  let mockRequest;
  let mockResponse;
  let fakeAirportsArray;
  ...
  beforeEach(() => {
    ...
    fakeAirportsArray = [
      { _id: '652f4c9c1b3d5e001f8a2b3c', name: 'Salt Lake City Airport', icaoCode: 'KSLC' },
      { _id: '652f4c9c1b3d5e001f8a2b3d', name: 'Provo Municipal Airport', icaoCode: 'KPVU' }
    ];
  });
  ...
  test('getAllAirports should return 200 and an array of records on success', async () => {
    // 2. Build the nested mock chain to match your code:
    // mongodb.getDatabase().db().collection('airports').find().toArray()
    const mockToArray = jest.fn().mockResolvedValue(fakeAirportsArray);
    ...
    await airportsController.getAllAirports(mockRequest, mockResponse);
    ...
    expect(mockResponse.json).toHaveBeenCalledWith(fakeAirportsArray);
  });

  test('getAllAirports should return 500 when database throws an error connection crash', async () => {
